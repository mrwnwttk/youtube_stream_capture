...
def v4():
	...
	for i in range(0, segments + 1):
		...
		#subprocess.Popen(["ffmpeg", "-hide_banner", "-loglevel", "panic", "-y", "-i", f"segments_{video_thing}/{i}_{video_thing}_audio.ts", "-c:a", "copy", f"segments_{video_thing}_audio/{i}_{video_thing}_audio.m4a"]).wait()
	...
	#subprocess.Popen(["ffmpeg", "-hide_banner", "-loglevel", "panic", "-y", "-f", "concat", "-safe", "0", "-i", f"list_{video_thing}_audio.txt", "-c:a", "copy", f"{video_thing}_audio_v1_ffmpeg.m4a"]).wait()
	...
	for i in range(0, segments + 1):
		...
		#subprocess.Popen(["ffmpeg", "-hide_banner", "-loglevel", "panic", "-y", "-i", f"segments_{video_thing}/{i}_{video_thing}_video.ts", "-c:a", "copy", f"segments_{video_thing}_video/{i}_{video_thing}_video.mkv"]).wait()
	#p.wait()
	print("", end='') # For the last \r
	#os.system("killall -9 ffmpeg")
	...
	#with open("list_{}_video.txt".format(video_thing), "w") as f:
		#for i in range(0, segments + 1):
			#f.write("file 'segments_{}_video/{}_{}_video.mkv'\n".format(video_thing, i, video_thing))
			#if os.path.isfile("segments_{}_video/{}_{}_video_1.mkv".format(video_thing, i, video_thing)):
			#	f.write("file 'segments_{}_video/{}_{}_video_1.mkv'\n".format(video_thing, i, video_thing))
